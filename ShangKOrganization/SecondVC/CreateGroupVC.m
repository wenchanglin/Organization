...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if (Manager.UserAddAdminId == nil) {
        Tbutton.text = @"请选择群管理";
    }else{
        Tbutton.text = Manager.UserAddAdminName;
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0&&indexPath.row==0) {
        ...
        [cell.GroupFirstPic setBackgroundImage:[UIImage imageNamed:@"我的@2x.png"] forState:UIControlStateNormal];
        ...
        cell.GroupInfoRow.ex_heightToKeyboard = 0;
        cell.GroupInfoRow.ex_moveView = self.view;
...
-(void)ChooseBtn:(UIButton *)bTN
{
    ...
    administra.WhereCome = @"ZiJiChuangJian";
        ...
        if (textView.text.length    != 0) {
            if (_selectedPhotos.count   != 0) {
                if (![Tbutton.text isEqualToString:@"请选择群管理"]) {
                    ...
                    NSArray *arr = @[@{@"id":Manager.UserAddAdminId}];
