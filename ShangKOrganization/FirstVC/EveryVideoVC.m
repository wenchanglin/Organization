...
@interface EveryVideoVC ()<UITableViewDelegate,UITableViewDataSource>
{
...
//        NSLog(@"%@ TTT %@",XModel.VideoStoreName,XModel.VideoStoreIntro);
        if (indexPath.section == 0) {
            ...
            cell.imageView.image = [UIImage imageNamed:@"图层-86@2x.png"];
        }
        return cell;
    }
    Xcell = [tableView dequeueReusableCellWithIdentifier:@"VideoCell"];
    if (!Xcell) {
        Xcell = [[EveryVideoCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"VideoCell"];
    }
