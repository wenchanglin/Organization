...
-(void)createUI
{
    ...
    imageView.image   = [UIImage imageNamed:@"组-3@2x.png"];
...
//            dispatch_async(dispatch_get_main_queue(), ^{
                [[AFHttpClient shareInstance]startRequestMethod:POST parameters:@{@"vipPriceId":PricrId,@"fkUserId":Manager.UUserId} url:UrL_BuyVip success:^(id responseObject) {
                    NSDictionary *Ditc = [responseObject objectForKey:@"data"];
                    NSString *VipId = [Ditc objectForKey:@"id"];
                    [[AFHttpClient shareInstance]startRequestMethod:POST parameters:@{@"orderId":VipId,@"clientType":@"org"} url:UrL_WxinPay success:^(id responseObject) {
...
//    NSLog(@"立即支付");
//    FbwManager *Manager = [FbwManager shareManager];
//    [[AFHttpClient shareInstance]startRequestMethod:POST parameters:@{@"vipPriceId":Price,@"fkUserId":Manager.UUserId} url:UrL_BuyVip success:^(id responseObject) {
////        [SVProgressHUD showSuccessWithStatus:[responseObject objectForKey:@"msg"]];
////        NSLog(@"立即支付%@",responseObject);
//    } failure:^(NSError *error) {
//        NSLog(@"支付失败");
//    }];
//}
