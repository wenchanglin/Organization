...
-(void)createHeadView
{
    ...
    NSArray *arr    = @[@"对号-拷贝@2x.png",@"对号-拷贝@2x.png"];
    NSArray *arrPic = @[@"对号@2x.png",@"对号@2x.png"];//
    NSArray *Title  = @[@"支付宝支付",@"微信支付"];
    NSArray *PicArr = @[@"图层-58@2x_19.png",@"图层-59@2x_76.png"];
    for (int i=0; i<2; i++) {
...
-(void)payChoose
{
    ...
    if([_type isEqualToString:@"alipay"]){
        ...
        [[AFHttpClient shareInstance]startRequestMethod:POST parameters:@{@"orderId":self.OrderId} url:UrL_ALiPayOrder success:^(id responseObject) {
            ...
            if ([self.PayMoneyChoose isEqualToString:@"AddObject"]) {
                Ali.ChooseTitle = @"AddObject";
            }else if ([self.PayMoneyChoose isEqualToString:@"AddActivity"]){
                Ali.ChooseTitle = @"AddActivity";
            }else if ([self.PayMoneyChoose isEqualToString:@"BuyVideo"]){
               Ali.ChooseTitle = @"BuyVideo";
            }else if ([self.PayMoneyChoose isEqualToString:@"SpBuy"]){
                Ali.ChooseTitle = @"SpBuy";
            }
...
//            return nil;
//        }
//    NSLog(@"我偷偷地笑%@",self.OrderId);
    NSLog(@"安装了微信，而且支持微信支付");
    [[AFHttpClient shareInstance]startRequestMethod:POST parameters:@{@"orderId":self.OrderId,@"clientType":@"org"} url:UrL_WxinPay success:^(id responseObject) {
