...
@interface MyVideosVC ()<UITableViewDelegate,UITableViewDataSource>
{
    UIView *_topView;
...
//下拉刷新
- (void)form {
    __weak MyVideosVC *weakSelf = self;
    if (_firstView) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag == 10) {
        ...
        cell.MyVideosTime.text = [NSString stringWithFormat:@"观看时间:%@",Model.MyVideosCreateime];
        ...
        [cell configWithModel:Model];
        dispatch_async(dispatch_get_main_queue(), ^{
        cell.MyVideosTime.text  = [NSString stringWithFormat:@"购买时间:%@",Model.MyVideosCreateime];
        ...
        imageView.image         = [UIImage imageNamed:@"下载@2x_36.png"];
    ...
    [cell configWithModel:Model];
    [cell.contentView addSubview:button];
    cell.MyVideosTime.text = [NSString stringWithFormat:@"下载时间:%@",Model.MyVideosCreateime];
    });
    return cell;
}
