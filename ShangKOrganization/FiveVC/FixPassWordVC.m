...
-(void)createUI
{
    ...
    for (int i=0; i<3; i++) {
        //WithFrame:CGRectMake(20, 100+i*60, kScreenWidth - 40, 40)];
        ...
        PIc.image = [UIImage imageNamed:@"图层-666Suo.png"];
...
-(void)LoginBtn:(UIButton *)loginBtn
{
    ...
    if ([NewTextField.text isEqualToString:QueRenNewPasswordTextField.text]) {
        ...
        [[AFHttpClient shareInstance]startRequestMethod:POST parameters:@{@"userName":[defaults objectForKey:@"UinfoUser"],@"roleType":@"organization",@"newPassword":NewTextField.text,@"oldPassword":OldTextField.text} url:UrL_NewPassWord success:^(id responseObject) {
